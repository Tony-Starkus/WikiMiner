...
public class DisambiguatorMOD {
	...
	public DisambiguatorMOD(Wikipedia wikipedia) throws IOException, Exception {
	 ...
	 * @throws Exception 
	 */
	public DisambiguatorMOD(Wikipedia wikipedia,  ArticleComparer comparer, TextProcessor textProcessor, double minSenseProbability, double minLinkProbability, int maxContextSize) throws Exception {
	...
	private void init(Wikipedia wikipedia, ArticleComparer comparer, TextProcessor textProcessor, double minSenseProbability, double minLinkProbability, int maxContextSize) throws Exception {
		...
		*/
		...
		if (wikipedia.getConfig().getCachePriority(DatabaseType.label) == null)
			Logger.getLogger(DisambiguatorMOD.class).warn("'label' database has not been cached, so this will run significantly slower than it needs to.") ;
		
		if (wikipedia.getConfig().getCachePriority(DatabaseType.pageLinksIn) == null)
			Logger.getLogger(DisambiguatorMOD.class).warn("'pageLinksIn' database has not been cached, so this will run significantly slower than it needs to.") ;
	}
	...
	public void trainMOD(ArticleSet articles, SnippetLength snippetLength, String datasetName, String output_dir, RelatednessCache rc) throws Exception{
		...
		disambiguatorMODFile.println("@relation LinkDisambiguator\n" + 
				"\n" + 
				"@attribute commonness numeric\n" + 
				"@attribute relatedness numeric\n" + 
				"@attribute contextQuality numeric\n" + 
				"@attribute isCorrectSense {TRUE,FALSE}\n\n"
				+ "@data");
	...
	public void saveTrainingData(File file) throws IOException, Exception {
		Logger.getLogger(DisambiguatorMOD.class).info("saving training data") ;
	...
	public void loadTrainingData(File file) throws Exception{
		Logger.getLogger(DisambiguatorMOD.class).info("loading training data") ;
		dataset = decider.createNewDataset();
		dataset.load(file) ;
	...
	public void saveClassifier(File file) throws IOException, Exception {
		Logger.getLogger(DisambiguatorMOD.class).info("saving classifier") ;
	...
	public void loadClassifier(File file) throws IOException, Exception {
		Logger.getLogger(DisambiguatorMOD.class).info("loading classifier") ;
	...
	public void buildClassifier(Classifier classifier) throws Exception {
		Logger.getLogger(DisambiguatorMOD.class).info("building classifier") ;
	...
	public Result<Integer> test(ArticleSet testSet, Wikipedia wikipedia2, SnippetLength snippetLength, RelatednessCache rc) throws SQLException, Exception{
		...
		ProgressTracker pn = new ProgressTracker(testSet.size(), "Testing", DisambiguatorMOD.class) ;
