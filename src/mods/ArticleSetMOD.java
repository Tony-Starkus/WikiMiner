 ...
 */
public class ArticleSetMOD extends ArrayList<Article> {
	 ...
	 */
	public ArticleSetMOD(File file, Wikipedia wikipedia) throws IOException{
	 ...
	 */
	public ArticleSetMOD(Wikipedia wikipedia, int size, Integer minInLinks, Integer minOutLinks, Double minLinkProportion, Double maxLinkProportion, Integer minWordCount, Integer maxWordCount, Double maxListProportion, Pattern mustMatch, Pattern mustNotMatch, Vector<Article> candidates , ArticleSetMOD exclude) {
	...
	public ArticleSetMOD getRandomSubset(int size) {
		...
		ArticleSetMOD subset = new ArticleSetMOD() ;
	...
	private void buildFromCandidates(Wikipedia wikipedia, Vector<Article> roughCandidates, int size, Integer minInLinks, Integer minOutLinks, Double minLinkProportion, Double maxLinkProportion, Integer minWordCount, Integer maxWordCount, Double maxListProportion, Pattern mustMatch, Pattern mustNotMatch, ArticleSetMOD exclude) {
		
		DecimalFormat df = new DecimalFormat("#0.00 %") ;
		
		int totalRoughCandidates = roughCandidates.size();
		
		ProgressTracker pn = new ProgressTracker(totalRoughCandidates, "Refining candidates (ETA is worst case)", ArticleSetMOD.class) ;
	...
	protected static Vector<Article> getRoughCandidates(Wikipedia wikipedia, Integer minInLinks, Integer minOutLinks)  {
		...
		ProgressTracker pn = new ProgressTracker(totalArticles, "Gathering rough candidates", ArticleSetMOD.class) ;
	...
	private boolean isArticleValid(Article art, Double minLinkProportion, Double maxLinkProportion, Integer minWordCount, Integer maxWordCount, Double maxListProportion, Pattern mustMatch, Pattern mustNotMatch, ArticleSetMOD exclude) {
			
		Logger.getLogger(ArticleSetMOD.class).debug("Evaluating " + art) ;
		
		
		//we don't want any disambiguations
		if (art.getType() == PageType.disambiguation) {
			Logger.getLogger(ArticleSetMOD.class).debug(" - rejected due to disambiguation") ;
			return false ;	
			
		}
		
		if (exclude != null && exclude.contains(art)) {
			Logger.getLogger(ArticleSetMOD.class).debug(" - rejected due to exclusion list") ;
			return false ;	
		}
		...
		if (mustMatch != null) {
			...
			if (!m.find()) {
				Logger.getLogger(ArticleSetMOD.class).debug(" - rejected due to mustMatch pattern") ;
				return false ;	
			}
		}
		...
		if (mustNotMatch != null) {
			...
			if (m.find()) {
				Logger.getLogger(ArticleSetMOD.class).debug(" - rejected due to mustNotMatch pattern") ;
				return false ;	
			}
		}
		...
		if (maxListProportion != null) {
			...
			if (listProportion > maxListProportion) {
				Logger.getLogger(ArticleSetMOD.class).debug(" - rejected for max list proportion " + (listProportion)) ;
				return false ;
			}
		}
		...
		if (minWordCount != null || maxWordCount != null || minLinkProportion != null || maxLinkProportion != null ) {
			...
			if (minWordCount != null && wordCount < minWordCount) {
				Logger.getLogger(ArticleSetMOD.class).debug(" - rejected for min wordcount " + (wordCount)) ;
				return false ;
				
			}
			
			if (maxWordCount != null && wordCount > maxWordCount) {
				Logger.getLogger(ArticleSetMOD.class).debug(" - rejected for max wordcount " + (wordCount)) ;
				return false ;
			}
			...
			if (minLinkProportion != null && linkProportion < minLinkProportion) {
				Logger.getLogger(ArticleSetMOD.class).debug(" - rejected for min link proportion " + (linkProportion)) ;
				return false ;
			}
			
			if (maxLinkProportion != null && linkProportion > maxLinkProportion) {
				Logger.getLogger(ArticleSetMOD.class).debug(" - rejected for max link proportion " + (linkProportion)) ;
				return false ;
			}
		}
