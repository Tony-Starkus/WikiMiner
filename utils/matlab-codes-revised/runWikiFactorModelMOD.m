...
path_output = "C:\Users\thali\Documents\MATLAB\Amostra por media\Amostra 4\colecoes\";
load(strcat(path_output, '3000_loose_wss-wooki_vfinal.mat'));

% Output train result
file_output = fopen(strcat(path_output, "results.txt"), "wt");
...
for k = [5 10 20 30 50 60 80 100 150 200]
    for EPOCHS = [10 50 100 150 200 250 300]
        fprintf(file_output, "k: %d | EPOCH: %d\n", k, EPOCHS);
        for epochFrac = [0.05 0.1 0.2 0.4] % fração de pares usados realmente para treinar do total de pares de treino
            % learning rates
            eta = [];
            eta.etaLatent = 1e-9; % learning rate for latent feature
            eta.etaRowBias = 1e-9; % learning rate for node bias
            eta.etaLatentScaler = 1e-9; % learning rate for scaler to latent features
            eta.etaBias = 1e-9;

            % penalization terms
            lambda = [];
            lambda.lambdaLatent = 1e-6; % regularization for node's latent vector U
            lambda.lambdaRowBias = 1e-6; % regularization for node's bias UBias
            lambda.lambdaLatentScaler = 1e-6; % regularization for scaling factors Lambda (in paper)

            lambda.lambdaPair = 1e-8; % regularization for weights on pair features
            lambda.lambdaBilinear = 1e-8; % regularization for weights on node features
            ...
            %% eta learning rates
            eta.etaPair = linkFeaturesPresent * 1e-9; % learning rate for pairs, when their features are present
            eta.etaBilinear = nodeFeaturesPresent * 1e-9; % learning rate for node, when their features are present
            eta.etaBias = linkFeaturesPresent * 1e-9; % learning rate for global bias, used when features are present
            ...
            for i=1:maxFolds
               rng(0+i,'twister'); % to ensure reproducible experiments
                ...
                Dtr = [];
                idsTr = read20perFold(path_output, i, 'train', trainFrac);
                ...
                idsTs = readFold(path_output, i, 'test');
            ...
            fprintf(file_output, '(epochFrac: %f) - avg: auc: %.4g f1: %.4g rmse: %.4g\n', epochFrac, ats_auc/maxFolds, ats_f1/maxFolds, ats_rmse/maxFolds);
